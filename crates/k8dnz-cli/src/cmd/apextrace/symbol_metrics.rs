...
pub const MAX_SYMBOL_CLASSES: usize = 4;

#[derive(Clone, Copy, Debug, Default, PartialEq)]
pub struct PerSymbolMetrics {
    pub support: u64,
    pub predicted: u64,
    pub tp: u64,
    pub fp: u64,
...
pub struct SymbolMetrics {
    pub class_count: u8,
    pub total: u64,
    pub target_hist: [u64; MAX_SYMBOL_CLASSES],
    pub pred_hist: [u64; MAX_SYMBOL_CLASSES],
    pub confusion: [[u64; MAX_SYMBOL_CLASSES]; MAX_SYMBOL_CLASSES],
    ...
    pub per_class: [PerSymbolMetrics; MAX_SYMBOL_CLASSES],
}

pub fn compute_symbol_metrics(target: &[u8], predicted: &[u8], class_count: u8) -> Result<SymbolMetrics> {
    if class_count == 0 || class_count as usize > MAX_SYMBOL_CLASSES {
        return Err(anyhow!(
            "symbol metrics: class_count must be in 1..={} (got {})",
            MAX_SYMBOL_CLASSES,
            class_count
        ));
    }
    if target.len() != predicted.len() {
        return Err(anyhow!(
            "symbol metrics: target len {} != predicted len {}",
            target.len(),
            predicted.len()
        ));
    }

    let mut target_hist = [0u64; MAX_SYMBOL_CLASSES];
    let mut pred_hist = [0u64; MAX_SYMBOL_CLASSES];
    let mut confusion = [[0u64; MAX_SYMBOL_CLASSES]; MAX_SYMBOL_CLASSES];
    let mut matches = 0u64;

    for (&t, &p) in target.iter().zip(predicted.iter()) {
        let ti = symbol_slot(t, class_count)?;
        let pi = symbol_slot(p, class_count)?;
    ...
    let (majority_class, majority_count) = argmax_hist(&target_hist, class_count);
    ...
    let (pred_dominant_class, pred_dominant_count) = argmax_hist(&pred_hist, class_count);
    ...
    let target_entropy_bits = entropy_bits(&target_hist, total, class_count);
    let pred_entropy_bits = entropy_bits(&pred_hist, total, class_count);

    let hist_l1 = (0..class_count as usize)
        .map(|idx| target_hist[idx].abs_diff(pred_hist[idx]))
        .sum::<u64>();
    ...
    let mut per_class = [PerSymbolMetrics::default(); MAX_SYMBOL_CLASSES];
    ...
    for cls in 0..class_count as usize {
        ...
        per_class[cls] = PerSymbolMetrics {
            support,
...
fn symbol_slot(v: u8, class_count: u8) -> Result<usize> {
    if v < class_count {
        Ok(v as usize)
    } else {
        Err(anyhow!(
            "symbol metrics: invalid class symbol {} for class_count {}",
            v,
            class_count
        ))
    }
...
fn argmax_hist(hist: &[u64; MAX_SYMBOL_CLASSES], class_count: u8) -> (u8, u64) {
    let mut best_idx = 0usize;
    let mut best_val = 0u64;
    for idx in 0..class_count as usize {
        let val = hist[idx];
        if val > best_val {
            best_idx = idx;
            best_val = val;
...
fn entropy_bits(hist: &[u64; MAX_SYMBOL_CLASSES], total: u64, class_count: u8) -> f64 {
    if total == 0 {
        return 0.0;
    }

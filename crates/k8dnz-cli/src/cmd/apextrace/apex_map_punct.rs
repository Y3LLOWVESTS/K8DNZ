...
use k8dnz_core::repr::{punct_lanes::{punct_class_label, PunctLanes}, text_norm};
use k8dnz_core::symbol::patch::PatchList;

use crate::cmd::apextrace::{ApexMapPunctArgs, ChunkSearchObjective, RenderFormat};
...
struct ChunkSearchSummary {
    ...
    mean_minority_macro_f1_pct: f64,
...
struct ApexMapPunctReport {
    ...
    chunk_mean_minority_macro_f1_pct: f64,
...
#[derive(Clone, Debug)]
struct ApexMapPunctRun {
    report: ApexMapPunctReport,
...
pub fn run_apex_map_punct(args: ApexMapPunctArgs) -> Result<()> {
    ...
    if punct.class_lane.is_empty() {
        return Err(anyhow!("apex-map-punct: empty normalized input"));
    }
    ...
    let global = brute_force_best_symbol_lane(&punct.class_lane, cfg, CLASS_COUNT)?;
    let global_patch = PatchList::from_pred_actual(&global.predicted, &punct.class_lane)
        .map_err(|e| anyhow!("apex-map-punct global patch build failed: {e}"))?;
    ...
    let mut runs = Vec::with_capacity(chunk_values.len().saturating_mul(boundary_band_values.len()).saturating_mul(field_margin_values.len()));
    for chunk_bytes in chunk_values {
        for &boundary_band in &boundary_band_values {
            for &field_margin in &field_margin_values {
                runs.push(run_apex_map_punct_once(
                    &args,
                    &norm,
                    &punct,
                    &global,
    ...
    if runs.is_empty() {
        return Err(anyhow!("apex-map-punct: no chunk sizes to run"));
    }
    ...
    if args.out_key.is_some() || args.out_pred.is_some() {
        if runs.len() > 1 {
            eprintln!(
                "apex-map-punct sweep selected best output run: chunk_bytes={} boundary_band={} field_margin={} compact_field_total_payload_exact={}",
    ...
    if let Some(path) = args.out_diag.as_deref() {
        ...
        eprintln!(
            "apextrace apex-map-punct diagnostics saved: {} rows={}",
            path,
...
fn run_apex_map_punct_once(
    args: &ApexMapPunctArgs,
    norm: &[u8],
    punct: &PunctLanes,
    global: &GenericLaneBest,
    global_patch_entries: usize,
    ...
    boundary_band: usize,
    field_margin: u64,
) -> Result<ApexMapPunctRun> {
    let target_metrics = compute_symbol_metrics(&punct.class_lane, &punct.class_lane, CLASS_COUNT)?;
    ...
    let chunk_patch = PatchList::from_pred_actual(&chunked.predicted, &punct.class_lane)
        .map_err(|e| anyhow!("apex-map-punct chunked patch build failed: {e}"))?;
    ...
    refine_cfg.desired_margin_add[1] = args.term_margin_add;
    refine_cfg.desired_margin_add[2] = args.pause_margin_add;
    refine_cfg.desired_margin_add[3] = args.wrap_margin_add;
    refine_cfg.dominant_share_ppm_min[1] = args.term_share_ppm_min;
    refine_cfg.dominant_share_ppm_min[2] = args.pause_share_ppm_min;
    refine_cfg.dominant_share_ppm_min[3] = args.wrap_share_ppm_min;
    ...
    let global_metrics = compute_symbol_metrics(&punct.class_lane, &global.predicted, CLASS_COUNT)?;
    let chunk_metrics = compute_symbol_metrics(&punct.class_lane, &chunked.predicted, CLASS_COUNT)?;
    let field_metrics = compute_symbol_metrics(&punct.class_lane, &field_predicted, CLASS_COUNT)?;

    let field_patch = PatchList::from_pred_actual(&field_predicted, &punct.class_lane)
        .map_err(|e| anyhow!("apex-map-punct field patch build failed: {e}"))?;
    ...
    if compact_manifest_decoded != compact_manifest {
        return Err(anyhow!("apex-map-punct compact manifest roundtrip mismatch"));
    }
    ...
    let report = ApexMapPunctReport {
        input: args.r#in.clone(),
        normalized_len: norm.len(),
        class_len: punct.class_lane.len(),
        other_len: punct.other_lane.len(),
        punct_len: punct.punct_lane.len(),

        majority_class: target_metrics.majority_class,
        majority_class_label: punct_class_label(target_metrics.majority_class),
        ...
        chunk_mean_minority_macro_f1_pct: chunk_summary.mean_minority_macro_f1_pct,
        ...
        field_pred_dominant_label: punct_class_label(field_metrics.pred_dominant_class),
        ...
        field_precision_term_pct: field_metrics.per_class[1].precision_pct,
        field_recall_term_pct: field_metrics.per_class[1].recall_pct,
        field_f1_term_pct: field_metrics.per_class[1].f1_pct,
        field_precision_pause_pct: field_metrics.per_class[2].precision_pct,
        field_recall_pause_pct: field_metrics.per_class[2].recall_pct,
        field_f1_pause_pct: field_metrics.per_class[2].f1_pct,
        field_precision_wrap_pct: field_metrics.per_class[3].precision_pct,
        field_recall_wrap_pct: field_metrics.per_class[3].recall_pct,
        field_f1_wrap_pct: field_metrics.per_class[3].f1_pct,
        ...
        field_term_applied: field_stats.applied_by_desired[1],
        field_pause_applied: field_stats.applied_by_desired[2],
        field_wrap_applied: field_stats.applied_by_desired[3],
...
fn refine_boundaries_with_diag(
    ...
    if base.len() != map.len() {
        return Err(anyhow!("apex-map-punct: refine base length mismatch"));
    }
    if target.len() != map.len() {
        return Err(anyhow!("apex-map-punct: refine target length mismatch"));
    }
...
fn best_run_index(runs: &[ApexMapPunctRun]) -> usize {
...
fn parse_usize_sweep_values(raw: Option<&str>, fallback: usize, label: &str) -> Result<Vec<usize>> {
    ...
    if out.is_empty() {
        return Err(anyhow!("apex-map-punct: {} sweep produced no values", label));
    }
    Ok(out)
}
...
fn brute_force_best_symbol_lane(target: &[u8], cfg: SearchCfg, class_count: u8) -> Result<GenericLaneBest> {
    if cfg.seed_step == 0 {
        return Err(anyhow!("apex-map-punct: seed_step must be >= 1"));
    }
    ...
    for quadrant in 0u8..=3 {
        ...
        while i < cfg.seed_count {
            ...
            let predicted = generate_bytes(&key)?
                ...
                .collect::<Vec<_>>();
            let diag = score_symbol_lane(target, &predicted, class_count)?;
            let cand = GenericLaneBest { key, predicted, diag };
            match &best {
                None => best = Some(cand),
    ...
    best.ok_or_else(|| anyhow!("apex-map-punct: search produced no candidates"))
}
...
fn brute_force_best_symbol_lane_chunked(
    ...
    if chunk_bytes == 0 {
        return Err(anyhow!("apex-map-punct: chunk_bytes must be >= 1"));
    }
    ...
    let mut sum_minority_macro_f1_pct = 0.0;
    ...
    while start < target.len() {
        ...
        let patch = PatchList::from_pred_actual(&best.predicted, slice)
            .map_err(|e| anyhow!("apex-map-punct chunk patch build failed: {e}"))?;
        let patch_bytes = patch.encode();

        sum_balanced_accuracy_pct += metrics.balanced_accuracy_pct;
        sum_macro_f1_pct += metrics.macro_f1_pct;
        sum_minority_macro_f1_pct += minority_macro_f1(&metrics, class_count);
    ...
    Ok((
        ...
        ChunkSearchSummary {
            ...
            mean_minority_macro_f1_pct: sum_minority_macro_f1_pct / denom,
...
fn brute_force_best_symbol_lane_objective(
    ...
    for quadrant in 0u8..=3 {
        ...
        while i < cfg.seed_count {
            ...
            if objective == ChunkSearchObjective::RawGuarded && raw_anchor_matches.saturating_sub(diag.score.matches) > raw_slack {
                i = i.saturating_add(1);
                continue;
            }
...
fn chunk_candidate_better(
    ...
    let cand_max_minority_delta_abs = max_minority_delta_abs(cand_metrics, class_count);
    let cur_max_minority_delta_abs = max_minority_delta_abs(cur_metrics, class_count);
    let cand_minority_macro_f1 = (minority_macro_f1(cand_metrics, class_count) * 1_000_000.0) as i64;
    let cur_minority_macro_f1 = (minority_macro_f1(cur_metrics, class_count) * 1_000_000.0) as i64;
    ...
    let cand_key = (
        ...
        cand_max_minority_delta_abs,
        std::cmp::Reverse(cand_minority_macro_f1),
    ...
    let cur_key = (
        ...
        cur_max_minority_delta_abs,
        std::cmp::Reverse(cur_minority_macro_f1),
...
fn minority_delta_abs(metrics: &SymbolMetrics, class_count: u8) -> i64 {
    let mut out = 0i64;
    for cls in 1..class_count as usize {
        out += ((metrics.pred_hist[cls] as i64) - (metrics.target_hist[cls] as i64)).abs();
    }
    out
}

fn max_minority_delta_abs(metrics: &SymbolMetrics, class_count: u8) -> i64 {
    let mut best = 0i64;
    for cls in 1..class_count as usize {
...
fn minority_macro_f1(metrics: &SymbolMetrics, class_count: u8) -> f64 {
    let mut sum = 0.0;
...
fn score_symbol_lane(target: &[u8], predicted: &[u8], class_count: u8) -> Result<GenericLaneDiagnostics> {
    if target.len() != predicted.len() {
        return Err(anyhow!(
            "apex-map-punct: target len {} != predicted len {}",
            target.len(),
            predicted.len()
        ));
    }
    ...
    for (idx, (&t, &p)) in target.iter().zip(predicted.iter()).enumerate() {
        target_hist[symbol_slot_local(t, class_count)?] += 1;
        pred_hist[symbol_slot_local(p, class_count)?] += 1;
...
fn save_outputs(
    ...
    if let Some(path) = out_key {
        ...
        eprintln!("saved apex-map-punct global key: {}", path);
        ...
        eprintln!("saved apex-map-punct chunk manifest: {}", chunk_path);
        ...
        eprintln!("saved apex-map-punct compact manifest csv: {}", compact_csv_path);
        ...
        eprintln!("saved apex-map-punct compact manifest bin: {}", compact_bin_path);
        ...
        eprintln!("saved apex-map-punct boundary pairs: {}", boundary_path);
    }

    if let Some(path) = out_pred {
        let global_ascii = render_punct_class_ascii(&global.predicted);
        ...
        eprintln!("saved apex-map-punct global class lane: {}", global_path);

        let chunked_ascii = render_punct_class_ascii(&chunked.predicted);
        ...
        eprintln!("saved apex-map-punct chunked class lane: {}", chunked_path);

        let field_ascii = render_punct_class_ascii(field_predicted);
        let field_path = format!("{}.field.txt", path);
        std::fs::write(&field_path, field_ascii.as_bytes()).with_context(|| format!("write {}", field_path))?;
        eprintln!("saved apex-map-punct field-refined class lane: {}", field_path);
    }
...
fn render_punct_class_ascii(predicted: &[u8]) -> String {
...
fn render_reports_txt(runs: &[ApexMapPunctRun]) -> String {
...
fn render_report_txt(row: &ApexMapPunctReport) -> String {
    let mut out = String::new();
    out.push_str(&format!("input={}\n", row.input));
    out.push_str(&format!("normalized_len={} class_len={} other_len={} punct_len={}\n", row.normalized_len, row.class_len, row.other_len, row.punct_len));
    out.push_str(&format!("majority_class={} majority_label={} majority_count={} majority_baseline_match_pct={:.6} target_entropy_bits={:.6}\n",
        row.majority_class,
    ...
    out.push_str(&format!("global_patch_entries={} global_patch_bytes={} global_total_payload_exact={} global_match_pct={:.6} global_match_vs_majority_pct={:.6} global_balanced_accuracy_pct={:.6} global_macro_f1_pct={:.6}\n",
    ...
    out.push_str(&format!("chunk_bytes={} chunk_count={} chunk_patch_entries={} chunk_patch_bytes={} chunk_total_payload_exact={} compact_manifest_bytes_exact={} compact_chunk_total_payload_exact={} chunk_match_pct={:.6} chunk_match_vs_majority_pct={:.6} chunk_balanced_accuracy_pct={:.6} chunk_macro_f1_pct={:.6} chunk_search_objective={} chunk_raw_slack={} chunk_mean_balanced_accuracy_pct={:.6} chunk_mean_macro_f1_pct={:.6} chunk_mean_minority_macro_f1_pct={:.6} chunk_mean_abs_minority_delta={:.6} chunk_majority_flip_count={} chunk_collapse_90_count={}\n",
        ...
        row.chunk_mean_minority_macro_f1_pct,
        row.chunk_mean_abs_minority_delta,
        row.chunk_majority_flip_count,
        row.chunk_collapse_90_count,
    ));
    out.push_str(&format!("field_source={} map_node_count={} map_depth_seen={} map_depth_shift={} map_max_depth_arg={} boundary_band={} boundary_delta={} field_margin={} term_margin_add={} pause_margin_add={} wrap_margin_add={} term_share_ppm_min={} pause_share_ppm_min={} wrap_share_ppm_min={}\n",
        row.field_source,
        row.map_node_count,
        row.map_depth_seen,
        row.map_depth_shift,
    ...
    out.push_str(&format!("field_pred_dominant_class={} field_pred_dominant_label={} field_pred_dominant_share_ppm={} field_pred_dominant_share_pct={:.6} field_pred_collapse_90_flag={}\n",
    ...
    out.push_str(&format!("field_precision_term_pct={:.6} field_recall_term_pct={:.6} field_f1_term_pct={:.6} field_precision_pause_pct={:.6} field_recall_pause_pct={:.6} field_f1_pause_pct={:.6} field_precision_wrap_pct={:.6} field_recall_wrap_pct={:.6} field_f1_wrap_pct={:.6}\n",
    ...
    out.push_str(&format!("field_overrides={} field_boundary_count={} field_touched_positions={} field_term_applied={} field_pause_applied={} field_wrap_applied={} diag_rows={}\n",
        row.field_overrides,
    ...
    out.push_str(&format!("delta_field_patch_vs_global={} delta_field_patch_vs_chunked={} delta_field_total_vs_global={} delta_field_total_vs_chunked={} delta_compact_chunk_total_vs_global={} delta_compact_field_total_vs_global={} delta_compact_field_total_vs_chunked={} delta_compact_field_total_vs_compact_chunked={}\n",
    ...
    out.push_str(&format!("target_hist=[{},{},{},{}] global_pred_hist=[{},{},{},{}] chunk_pred_hist=[{},{},{},{}] field_pred_hist=[{},{},{},{}]\n",
...
"
    ...
    for run in runs {
        ...
        let cells = vec![
            ...
            row.punct_len.to_string(),
            row.majority_class.to_string(),
            punct_class_label(row.majority_class).to_string(),
            ...
            fmt6(row.chunk_mean_minority_macro_f1_pct),
            ...
            punct_class_label(row.field_pred_dominant_class).to_string(),
...
fn render_diag_csv(runs: &[ApexMapPunctRun]) -> String {
...
fn print_summary(out_path: Option<&str>, format: RenderFormat, runs: &[ApexMapPunctRun], best_idx: usize) {
    let best = &runs[best_idx].report;
    match out_path {
        Some(path) => {
            eprintln!(
                "apextrace apex-map-punct {:?} saved: {} (runs={} best_chunk={} best_boundary_band={} best_field_margin={} compact_field_total_payload_exact={} field_match_pct={:.6})",
                format,
                path,
                runs.len(),
                best.chunk_bytes,
        ...
        None => {
            eprintln!(
                "apextrace apex-map-punct {:?}: runs={} best_chunk={} best_boundary_band={} best_field_margin={} compact_field_total_payload_exact={} field_match_pct={:.6}",
                format,
                runs.len(),
                best.chunk_bytes,
                best.boundary_band,
...
fn symbol_slot_local(v: u8, class_count: u8) -> Result<usize> {
    if v < class_count {
        ...
        Err(anyhow!("apex-map-punct: invalid class symbol {}", v))
    }
}

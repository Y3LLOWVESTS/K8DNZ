...
use crate::cmd::apextrace::{ApexLaneLawSplitFreezeArgs, ChunkSearchObjective, RenderFormat};
...
struct FreezeSummary {
    ...
    split_chunk_candidates: String,
    split_chunk_candidate_count: usize,
    split_chunk_candidate_source: String,
...
pub fn run_apex_lane_law_split_freeze(args: ApexLaneLawSplitFreezeArgs) -> Result<()> {
    if args.inputs.is_empty() {
        return Err(anyhow!("apex-lane-law-split-freeze requires at least one --in input"));
    }

    let exe = env::current_exe().context("resolve current executable for apex-lane-law-split-freeze")?;
    ...
    let temp_dir = make_temp_dir("apex_lane_law_split_freeze")?;
    ...
    for report in &reports {
        let input_bytes = fs::read(&report.input)
            .with_context(|| format!("read input for freeze eval {}", report.input))?;
        ...
        let mut frozen_windows = Vec::with_capacity(report.windows.len());
        ...
        for window in &report.windows {
            ...
            if is_target_window {
                ...
                let window_path = temp_dir.join(format!(
                    "split_{}_window_{:04}_{:08}_{:08}.bin",
                ...
                fs::write(&window_path, slice)
                    .with_context(|| format!("write split freeze slice {}", window_path.display()))?;
                let frozen = run_child_frozen_apex_map_lane(&exe, &args, &freeze, &window_path)
                    .with_context(|| {
                        format!(
                            "run split target apex-map-lane input={} window_idx={}",
                            report.input, window.window_idx
                        )
                    })?;

                let delta_payload_exact = (frozen.compact_field_total_payload_exact as i64)
                ...
                all_window_deltas.push(WindowDeltaRow {
                    ...
                    frozen_payload_exact: frozen.compact_field_total_payload_exact,
                    delta_payload_exact,
                    searched_match_pct: window.field_match_pct,
                    frozen_match_pct: frozen.field_match_pct,
                    searched_chunk_bytes: window.chunk_bytes,
                    frozen_chunk_bytes: frozen.search.chunk_bytes,
                    searched_chunk_search_objective: window.chunk_search_objective.clone(),
                    frozen_chunk_search_objective: frozen.search.chunk_search_objective.clone(),
                    searched_chunk_raw_slack: window.chunk_raw_slack,
                    frozen_chunk_raw_slack: frozen.search.chunk_raw_slack,
                    searched_boundary_band: searched_law.boundary_band,
                    frozen_boundary_band: frozen.law.boundary_band,
                    searched_field_margin: searched_law.field_margin,
                    frozen_field_margin: frozen.law.field_margin,
                    searched_newline_demote_margin: searched_law.newline_demote_margin,
                    frozen_newline_demote_margin: frozen.law.newline_demote_margin,
                });

                frozen_windows.push(FrozenWindowRow {
                    window_idx: window.window_idx,
                    start: window.start,
                    end: window.end,
                    span_bytes: window.span_bytes,
                ...
                all_window_deltas.push(WindowDeltaRow {
                    ...
                    searched_global_law_id: global_law_id,
                    searched_payload_exact: window.compact_field_total_payload_exact,
                    frozen_payload_exact: window.compact_field_total_payload_exact,
                    delta_payload_exact: 0,
                    searched_match_pct: window.field_match_pct,
                    frozen_match_pct: window.field_match_pct,
                    searched_chunk_bytes: window.chunk_bytes,
                    frozen_chunk_bytes: window.chunk_bytes,
                    searched_chunk_search_objective: window.chunk_search_objective.clone(),
                    frozen_chunk_search_objective: window.chunk_search_objective.clone(),
                    searched_chunk_raw_slack: window.chunk_raw_slack,
                    frozen_chunk_raw_slack: window.chunk_raw_slack,
                    searched_boundary_band: searched_law.boundary_band,
                    frozen_boundary_band: searched_law.boundary_band,
                    searched_field_margin: searched_law.field_margin,
                    frozen_field_margin: searched_law.field_margin,
                    searched_newline_demote_margin: searched_law.newline_demote_margin,
                    frozen_newline_demote_margin: searched_law.newline_demote_margin,
                });
        ...
        let frozen_segments = build_frozen_segments(&frozen_windows, args.merge_gap_bytes);
        let frozen_laws = build_frozen_law_summaries(&frozen_windows, &frozen_segments);
        let law_index_by_id = frozen_laws
            .iter()
            .enumerate()
        ...
        let frozen_shared_header_bytes_exact = encode_replay_header(&replay_header).len();
        let frozen_law_dictionary_bytes_exact = encode_law_dictionary(&frozen_laws).len();
        let frozen_window_path_bytes_exact =
            encode_window_path(report.input_bytes, &frozen_windows, &law_index_by_id).len();
        let frozen_segment_path_bytes_exact =
            encode_segment_path(report.input_bytes, &frozen_segments, &law_index_by_id).len();
        let (frozen_selected_path_mode, frozen_selected_path_bytes_exact) =
            if frozen_segment_path_bytes_exact <= frozen_window_path_bytes_exact {
                ("segment".to_string(), frozen_segment_path_bytes_exact)
            } else {
                ("window".to_string(), frozen_window_path_bytes_exact)
            };
        let frozen_local_compact_payload_bytes_exact = frozen_windows
            .iter()
            .map(|row| row.compact_field_total_payload_exact)
            .sum::<usize>();
        let frozen_total_piecewise_payload_exact = frozen_local_compact_payload_bytes_exact
            .saturating_add(frozen_shared_header_bytes_exact)
            .saturating_add(frozen_law_dictionary_bytes_exact)
            .saturating_add(frozen_selected_path_bytes_exact);
        ...
        file_summaries.push(FrozenFileSummary {
            ...
            frozen_local_compact_payload_bytes_exact,
            frozen_shared_header_bytes_exact,
            frozen_law_dictionary_bytes_exact,
            frozen_window_path_bytes_exact,
            frozen_segment_path_bytes_exact,
            frozen_selected_path_mode,
            frozen_selected_path_bytes_exact,
    ...
    if !args.keep_temp_dir {
        if let Err(err) = fs::remove_dir_all(&temp_dir) {
            eprintln!(
                "apex-lane-law-split-freeze: warning could not remove temp_dir={} err={}",
                temp_dir.display(),
                err
            );
        }
    } else {
        eprintln!("apex-lane-law-split-freeze: temp_dir={}", temp_dir.display());
    }

    let mut best_gain: Option<&WindowDeltaRow> = None;
    let mut worst_loss: Option<&WindowDeltaRow> = None;
    for row in &all_window_deltas {
        if row.delta_payload_exact < 0 {
            let replace = best_gain
                .map(|cur| row.delta_payload_exact < cur.delta_payload_exact)
                .unwrap_or(true);
            if replace {
                best_gain = Some(row);
            }
    ...
    let summary = FreezeSummary {
        ...
        split_chunk_candidates: join_usize_csv(&freeze.chunk_candidates),
        split_chunk_candidate_count: freeze.chunk_candidates.len(),
        split_chunk_candidate_source: freeze.chunk_candidate_source.clone(),
        ...
        searched_total_piecewise_payload_exact: file_summaries
            ...
            .sum::<usize>(),
        frozen_total_piecewise_payload_exact: file_summaries
            .iter()
            .map(|f| f.frozen_total_piecewise_payload_exact)
            .sum::<usize>(),
        delta_total_piecewise_payload_exact: file_summaries
            .iter()
            .map(|f| f.delta_total_piecewise_payload_exact)
            .sum::<i64>(),
        all_window_count: file_summaries.iter().map(|f| f.all_window_count).sum::<usize>(),
        target_window_count: file_summaries
            .iter()
            .map(|f| f.target_window_count)
            .sum::<usize>(),
        searched_target_window_payload_exact: file_summaries
            .iter()
            .map(|f| f.searched_target_window_payload_exact)
            .sum::<usize>(),
        frozen_target_window_payload_exact: file_summaries
            .iter()
            .map(|f| f.frozen_target_window_payload_exact)
            .sum::<usize>(),
        delta_target_window_payload_exact: file_summaries
            .iter()
            .map(|f| f.delta_target_window_payload_exact)
            .sum::<i64>(),
        improved_window_count: file_summaries
            .iter()
            .map(|f| f.improved_window_count)
            .sum::<usize>(),
        equal_window_count: file_summaries
            .iter()
            .map(|f| f.equal_window_count)
            .sum::<usize>(),
        worsened_window_count: file_summaries
            .iter()
            .map(|f| f.worsened_window_count)
            .sum::<usize>(),
        ...
        worsened_target_window_count: file_summaries
            ...
            .sum::<usize>(),
        best_gain_input: best_gain.map(|r| r.input.clone()).unwrap_or_default(),
        best_gain_window_idx: best_gain.map(|r| r.window_idx).unwrap_or(0),
        best_gain_delta_payload_exact: best_gain.map(|r| r.delta_payload_exact).unwrap_or(0),
        worst_loss_input: worst_loss.map(|r| r.input.clone()).unwrap_or_default(),
        worst_loss_window_idx: worst_loss.map(|r| r.window_idx).unwrap_or(0),
        worst_loss_delta_payload_exact: worst_loss.map(|r| r.delta_payload_exact).unwrap_or(0),
    };
    ...
    if let Some(path) = args.out.as_deref() {
        eprintln!(
            "apex-lane-law-split-freeze: out={} files={} target={} searched_total_piecewise_payload_exact={} frozen_total_piecewise_payload_exact={} delta_total_piecewise_payload_exact={}",
            path,
            summary.file_count,
        ...
        eprintln!(
            "apex-lane-law-split-freeze: files={} target={} searched_total_piecewise_payload_exact={} frozen_total_piecewise_payload_exact={} delta_total_piecewise_payload_exact={}",
            summary.file_count,
            summary.target_global_law_id,
            summary.searched_total_piecewise_payload_exact,
            summary.frozen_total_piecewise_payload_exact,
...
fn select_target_profile<'a>(
    ...
    if profiles.is_empty() {
        return Err(anyhow!("apex-lane-law-split-freeze found no shared laws to freeze"));
    }
    ...
    profiles
        ...
        .ok_or_else(|| anyhow!("apex-lane-law-split-freeze could not select dominant law"))
}


fn build_freeze_config(
    args: &ApexLaneLawSplitFreezeArgs,
    target: &LawProfile,
    chunk_candidates: Vec<usize>,
    chunk_candidate_source: String,
) -> Result<FreezeConfig> {
    ...
    if let Some(v) = args.split_chunk_search_objective {
        search.chunk_search_objective = chunk_search_objective_name(v).to_string();
    }
    if let Some(v) = args.split_chunk_raw_slack {
...
fn select_chunk_candidates(
    ...
    if let Some(raw) = &args.split_chunk_sweep {
        let mut values = parse_usize_csv(raw)
            .with_context(|| format!("parse --split-chunk-sweep {}", raw))?;
        if values.is_empty() {
            return Err(anyhow!("--split-chunk-sweep must contain at least one chunk size"));
        }
...
fn render_txt(
    ...
    push_line(&mut out, "split_chunk_candidates", &summary.split_chunk_candidates);
    push_line(
        &mut out,
        "split_chunk_candidate_count",
        summary.split_chunk_candidate_count,
    );
    push_line(
        &mut out,
        "split_chunk_candidate_source",
        &summary.split_chunk_candidate_source,
    );
...
mod tests {
    ...
    fn sample_args() -> ApexLaneLawSplitFreezeArgs {
        ApexLaneLawSplitFreezeArgs {
            ...
            split_chunk_sweep: None,
            split_chunk_search_objective: None,
            split_chunk_raw_slack: None,
            global_law_id: None,
            top_rows: 4,
            keep_temp_dir: false,

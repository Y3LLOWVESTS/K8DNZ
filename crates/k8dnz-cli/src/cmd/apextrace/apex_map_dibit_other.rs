...
use crate::cmd::apextrace::{ApexMapDibitOtherArgs, ChunkSearchObjective, RenderFormat};
...
struct DibitRunRow {
    ...
    map_depth_shift: u8,
    oracle_full_byte_match_pct: f64,
...
pub fn run_apex_map_dibit_other(args: ApexMapDibitOtherArgs) -> Result<()> {
    ...
    if input.is_empty() {
        return Err(anyhow!("apex-map-dibit-other: input is empty"));
    }
    let norm = text_norm::normalize_newlines(&input);
    let ws = WsLanes::split(&norm);
    if ws.other_lane.is_empty() {
        return Err(anyhow!("apex-map-dibit-other: other_lane is empty after whitespace split"));
    }
    let target_bytes = ws.other_lane.clone();
    let target_symbols = bytes_to_dibit_symbols(&target_bytes)?;
    ...
    let global = brute_force_best_dibit(&target_bytes, cfg)?;
    ...
    let global_patch = PatchList::from_pred_actual(&global.predicted, &target_symbols)
        .map_err(|e| anyhow!("apex-map-dibit-other global patch build failed: {e}"))?;
    ...
    for &chunk_bytes in &chunk_values {
        ...
        let chunk_patch = PatchList::from_pred_actual(&chunked.predicted, &target_symbols)
            .map_err(|e| anyhow!("apex-map-dibit-other chunk patch build failed: {e}"))?;
        ...
        for &boundary_band in &boundary_band_values {
            for &field_margin in &field_margin_values {
                ...
                let field_patch = PatchList::from_pred_actual(&field_predicted, &target_symbols)
                    .map_err(|e| anyhow!("apex-map-dibit-other field patch build failed: {e}"))?;
                ...
                let field_other_bytes = dibit_symbols_to_bytes(&field_predicted)?;
                let oracle_full = WsLanes {
                    total_len: ws.total_len,
                    class_lane: ws.class_lane.clone(),
                    other_lane: field_other_bytes,
                }
                .reconstruct()
                .ok_or_else(|| anyhow!("apex-map-dibit-other: reconstruct failed"))?;
                let oracle_full_matches = count_exact_matches(&norm, &oracle_full)?;
                ...
                rows.push(DibitRunArtifacts {
                    row: DibitRunRow {
                        ...
                        other_len: target_bytes.len(),
                        chunk_bytes,
                        boundary_band,
                        field_margin,
                        global_match_pct: global_metrics.raw_match_pct,
                        global_byte_match_pct: byte_match_pct(global.diag.byte_matches, target_bytes.len() as u64),
                        ...
                        chunk_byte_match_pct: byte_match_pct(chunked.diag.byte_matches, target_bytes.len() as u64),
                        ...
                        oracle_full_byte_match_pct: byte_match_pct(oracle_full_matches, norm.len() as u64),
                        ...
                        field_byte_match_pct: byte_match_pct(count_byte_matches(&target_symbols, &field_predicted)?, target_bytes.len() as u64),
    ...
    if rows.is_empty() {
        return Err(anyhow!("apex-map-dibit-other: no rows produced"));
    }

    let best_idx = select_best_row(&rows).ok_or_else(|| anyhow!("apex-map-dibit-other: no best row selected"))?;
    ...
    if let Some(path) = args.out_pred.as_deref() {
        ...
        let oracle_full = WsLanes { total_len: best.row.normalized_len, class_lane: ws.class_lane.clone(), other_lane: bytes }.reconstruct().ok_or_else(|| anyhow!("apex-map-dibit-other: reconstruct failed"))?;
        let oracle_path = format!("{}.oracle.bin", path);
        std::fs::write(&oracle_path, oracle_full).with_context(|| format!("write {}", oracle_path))?;
    }
    if let Some(path) = args.out_diag.as_deref() {
        std::fs::write(path, render_rows_csv(&rows)).with_context(|| format!("write {}", path))?;
        eprintln!(
            "apextrace apex-map-dibit-other sweep diagnostics saved: {} rows={}",
            path,
            rows.len()
        );
    }
...
fn bytes_to_dibit_symbols(bytes: &[u8]) -> Result<Vec<u8>> {
    bytes_to_quats(bytes)
        .map_err(|e| anyhow!("apex-map-dibit-other bytes_to_quats failed: {e}"))?
        .into_iter()
        .map(|q| {
            q.checked_sub(1)
                .ok_or_else(|| anyhow!("apex-map-dibit-other invalid quat {}", q))
        })
        .collect()
}
...
fn dibit_symbols_to_bytes(symbols: &[u8]) -> Result<Vec<u8>> {
    let quats = symbols
        ...
        .map(|&sym| {
            if sym > 3 {
                Err(anyhow!("apex-map-dibit-other invalid dibit symbol {}", sym))
            } else {
                Ok(sym + 1)
            }
        })
        .collect::<Result<Vec<_>>>()?;
    quats_to_bytes(&quats).map_err(|e| anyhow!("apex-map-dibit-other quats_to_bytes failed: {e}"))
}

fn brute_force_best_dibit(input_bytes: &[u8], cfg: SearchCfg) -> Result<DibitBest> {
    if cfg.seed_step == 0 {
        return Err(anyhow!("apex-map-dibit-other: seed_step must be >= 1"));
    }
    ...
    for quadrant in 0u8..=3 {
        ...
        while i < cfg.seed_count {
            ...
            let predicted = generate_quats(&key)
                .map_err(|e| anyhow!("apex-map-dibit-other generate_quats failed: {e}"))?
                .into_iter()
                .map(|q| q - 1)
                .collect::<Vec<_>>();
    ...
    best.ok_or_else(|| anyhow!("apex-map-dibit-other: search produced no candidates"))
}
...
fn brute_force_best_dibit_chunked(
    ...
    if chunk_bytes == 0 {
        return Err(anyhow!("apex-map-dibit-other: chunk_bytes must be >= 1"));
    }
...
fn score_dibit_symbols(target: &[u8], predicted: &[u8]) -> Result<DibitDiagnostics> {
    if target.len() != predicted.len() {
        return Err(anyhow!(
            "apex-map-dibit-other: target len {} != predicted len {}",
            target.len(),
            predicted.len()
        ));
    }
...
fn compute_dibit_metrics(target: &[u8], predicted: &[u8]) -> Result<DibitMetrics> {
    if target.len() != predicted.len() {
        return Err(anyhow!(
            "apex-map-dibit-other metrics: target len {} != predicted len {}",
            target.len(),
            predicted.len()
        ));
    }
...
fn count_byte_matches(target: &[u8], predicted: &[u8]) -> Result<u64> {
    if target.len() != predicted.len() {
        return Err(anyhow!(
            "apex-map-dibit-other byte match: target len {} != predicted len {}",
            target.len(),
            predicted.len()
        ));
    }
    if target.len() % 4 != 0 {
        return Err(anyhow!(
            "apex-map-dibit-other byte match: dibit len {} is not divisible by 4",
            target.len()
        ));
    }
    let mut matches = 0u64;
...
fn dibit_slot(v: u8) -> Result<usize> {
    match v {
        0..=3 => Ok(v as usize),
        _ => Err(anyhow!("apex-map-dibit-other invalid dibit symbol {}", v)),
    }
}
    ...
    fn from_chunked(chunked: &DibitChunkedBest) -> Result<Self> {
        ...
        for chunk in &chunked.chunks {
            if chunk.key.recipe_seed != recipe_seed {
                return Err(anyhow!(
                    "apex-map-dibit-other compact manifest requires shared recipe_seed across chunks"
                ));
            }
        }
        Ok(Self {
...
fn render_rows_txt(input: &[u8], target_symbols: &[u8], rows: &[DibitRunArtifacts], best_idx: usize) -> String {
    ...
    for (idx, row) in rows.iter().enumerate() {
        let r = &row.row;
                        push_kv(&mut out, "input_bytes", r.input_bytes);
        push_kv(&mut out, "normalized_len", r.normalized_len);
        push_kv(&mut out, "class_len", r.class_len);
        push_kv(&mut out, "other_len", r.other_len);
        ...
        push_kv(&mut out, "field_match_pct", format6(r.field_match_pct));
                push_kv(&mut out, "field_other_byte_match_pct", format6(r.field_byte_match_pct));
        push_kv(&mut out, "oracle_full_byte_match_pct", format6(r.oracle_full_byte_match_pct));
        ...
        if idx == best_idx {
            out.push_str("apextrace apex-map-dibit-other best_row=true\n");
        }
        if idx + 1 != rows.len() {
            out.push_str("\n---\n");
        }
    ...
    out.push_str("\n---\n");
    out.push_str(&format!(
        "apextrace apex-map-dibit-other best: chunk_bytes={} boundary_band={} field_margin={} compact_field_total_payload_exact={} field_patch_bytes={} field_match_pct={} field_other_byte_match_pct={} oracle_full_byte_match_pct={} majority_baseline_match_pct={} field_match_vs_majority_pct={} field_balanced_accuracy_pct={} field_macro_f1_pct={} field_pred_dominant_label={} field_pred_dominant_share_pct={} field_pred_collapse_90_flag={}\n",
        best.chunk_bytes,
        best.boundary_band,
        best.field_margin,
        ...
        format6(best.oracle_full_byte_match_pct),
        format6(best.field_match_pct - best.field_match_vs_majority_pct),
...
fn render_rows_csv(rows: &[DibitRunArtifacts]) -> String {
    ...
    for row in rows {
        ...
        let cols = vec![
            ...
            r.class_len.to_string(),
            r.other_len.to_string(),
            ...
            format!("{:.6}", r.oracle_full_byte_match_pct),
...
fn count_exact_matches(target: &[u8], predicted: &[u8]) -> Result<u64> {
    if target.len() != predicted.len() {
        return Err(anyhow!("apex-map-dibit-other exact match len {} != {}", target.len(), predicted.len()));
    }
    Ok(target.iter().zip(predicted.iter()).filter(|(a, b)| a == b).count() as u64)
}

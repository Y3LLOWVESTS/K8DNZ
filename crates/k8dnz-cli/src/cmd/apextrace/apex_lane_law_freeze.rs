...
use crate::cmd::apextrace::{ApexLaneLawFreezeArgs, ChunkSearchObjective, RenderFormat};
...
pub fn run_apex_lane_law_freeze(args: ApexLaneLawFreezeArgs) -> Result<()> {
    if args.inputs.is_empty() {
        return Err(anyhow!("apex-lane-law-freeze requires at least one --in input"));
    }

    let exe = env::current_exe().context("resolve current executable for apex-lane-law-freeze")?;
    ...
    let freeze = build_freeze_config(&args, target_profile)?;

    let temp_dir = make_temp_dir("apex_lane_law_freeze")?;
    ...
    for report in &reports {
        ...
        for window in &report.windows {
            ...
            let window_path = temp_dir.join(format!(
                "freeze_{}_window_{:04}_{:08}_{:08}.bin",
            ...
            fs::write(&window_path, slice)
                .with_context(|| format!("write freeze slice {}", window_path.display()))?;
            let frozen = run_child_frozen_apex_map_lane(&exe, &args, &freeze, &window_path)
                .with_context(|| {
                    format!(
                        "run frozen apex-map-lane input={} window_idx={}",
    ...
    if !args.keep_temp_dir {
        if let Err(err) = fs::remove_dir_all(&temp_dir) {
            eprintln!(
                "apex-lane-law-freeze: warning could not remove temp_dir={} err={}",
                temp_dir.display(),
                err
            );
        }
    } else {
        eprintln!("apex-lane-law-freeze: temp_dir={}", temp_dir.display());
    }
    ...
    if let Some(path) = args.out.as_deref() {
        eprintln!(
            "apex-lane-law-freeze: out={} files={} target={} searched_total_piecewise_payload_exact={} frozen_total_piecewise_payload_exact={} delta_total_piecewise_payload_exact={}",
            path,
            summary.file_count,
        ...
        eprintln!(
            "apex-lane-law-freeze: files={} target={} searched_total_piecewise_payload_exact={} frozen_total_piecewise_payload_exact={} delta_total_piecewise_payload_exact={}",
            summary.file_count,
            summary.target_global_law_id,
            summary.searched_total_piecewise_payload_exact,
            summary.frozen_total_piecewise_payload_exact,
...
fn select_target_profile<'a>(
    ...
    if profiles.is_empty() {
        return Err(anyhow!("apex-lane-law-freeze found no shared laws to freeze"));
    }
    ...
    profiles
        ...
        .ok_or_else(|| anyhow!("apex-lane-law-freeze could not select dominant law"))
}

fn build_freeze_config(args: &ApexLaneLawFreezeArgs, target: &LawProfile) -> Result<FreezeConfig> {
    ...
    if let Some(v) = args.freeze_chunk_bytes {
        search.chunk_bytes = v;
    }
    if let Some(v) = args.freeze_chunk_search_objective {
        search.chunk_search_objective = chunk_search_objective_name(v).to_string();
    }
    if let Some(v) = args.freeze_chunk_raw_slack {
...
mod tests {
    use super::{
        ...
        parse_manifest_txt, ReplayLawTuple,
    };
    use crate::cmd::apextrace::{ApexLaneLawFreezeArgs, ChunkSearchObjective, RenderFormat};
    ...
    fn sample_args() -> ApexLaneLawFreezeArgs {
        ApexLaneLawFreezeArgs {
            ...
            freeze_chunk_bytes: None,
            freeze_chunk_search_objective: None,
            freeze_chunk_raw_slack: None,
            global_law_id: None,
            top_rows: 4,
            keep_temp_dir: false,
    ...
    fn build_freeze_config_defaults_to_dominant_profile() {
        ...
        let freeze = build_freeze_config(&sample_args(), g1).expect("freeze config");
